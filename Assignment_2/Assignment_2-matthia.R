...
run.trials <- function(int1, int2, DM, clock.time)
{
...
experiment <- function(sub, trials)
{
  for (i in 1:num.subjects)
  {
    ...
    DM <- create.dm(25,num.trials*num.sessions) # size of the DM
    conditions <- sample(1:3)
    
    for (cond in conditions) {
      if (condition.type == 1)  #short experimental condition
      {
        run.trials(494, 847, DM, clock.time)
      }
      
      else if (condition.type == 2) #intermidiate experimental condition
      {
        run.trials(671, 1023, DM, clock.time)
      }
      
      else
      {
        run.trials(847, 1200, DM, clock.time)  #long experimental condition
      }
    }
  }
}
